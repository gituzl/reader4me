...
// NOTE: guard `process` because this module also runs on the Cloudflare Workers
// runtime (Pages Functions), where `process` is not defined.
const DEFAULT_MODEL = 'Qwen/Qwen2.5-32B-Instruct';
const SF_MODEL =
  (typeof process !== 'undefined' && process.env && process.env.SF_MODEL) || DEFAULT_MODEL;
...
async function lookup(opts) {
  ...
  const model = (opts && opts.model) || SF_MODEL;
